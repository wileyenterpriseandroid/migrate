...
public class DBKVStore implements KVStore {
    ...
    private static final String getSql = "select dataKey, className, value, updateTime, version, deleted from kv_table where namespace=? and dataKey = ?";
    private static final String getWithVersionSql = getSql + " and version = ?";

    // fully delete the item and its data
    private static final String DELETE_SQL = "delete from kv_table where namespace=? and dataKey = ?";
    ...
    private static final String DELETE_SOFT_UPDATE_SQL = "update kv_table set value = NULL, updateTime=?, deleted=1 where namespace=? and (deleted is NULL or deleted=0) and dataKey=?";
    private static final String UPDATE_SQL = "update kv_table set value = ?, updateTime=?, version=? where namespace = ? and dataKey=?";
    ...
    private static final String FIND_SQL = "select namespace, dataKey, className, value, updateTime, version, deleted from kv_table where (namespace=?) and (classname = ?) and (updateTime > ?) limit ?,?";
    ...
    @Override
    public void delete(final String namespace, final String key, boolean isPermanent, long now)
            throws IOException
    {
        if (isPermanent) {
            jdbcTemplate.update(DELETE_SQL, new Object[] {namespace, key});
        } else {
            jdbcTemplate.update(DELETE_SOFT_UPDATE_SQL, new Object[] {now, namespace, key});
        }
    }
